...
const CheckInRecordSchema = new mongoose.Schema({
    ...
    thoiGianMuonCheckIn: {
        type: Number, // Minutes late for check-in
        default: 0
    },
    thoiGianSomCheckOut: {
        type: Number, // Minutes early for check-out
        default: 0
    },
    sessionDuration: {
...
CheckInRecordSchema.index({ hoiVien: 1, checkInTime: -1 });
CheckInRecordSchema.index({ buoiTap: 1, hoiVien: 1 });
CheckInRecordSchema.index({ checkInTime: -1 });

// Method to calculate session duration
CheckInRecordSchema.methods.calculateDuration = function () {
...
CheckInRecordSchema.statics.findTodayRecords = function (hoiVienId) {
